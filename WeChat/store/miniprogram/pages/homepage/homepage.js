...
Page({
  ...
  data: {
    ...
    offline:false,   //打烊了
    isShow:true,
    imgUrls:['https://m.360buyimg.com/mobilecms/s843x843_jfs/t1/152869/36/733/129848/5f6da644E8c640b50/35117f17f08456ad.jpg!q70.dpg.webp',
    ...
    app.getInfoWhere('fruit-board',{_id: id},e =>
     {
        ...
        newCartItem.num=1;
        if(app.isNotRepeteToCart(newCartItem)){
  ...
  typeSwitch(e){
    ...
    wx.showLoading({
     ...
     switch(e.currentTarget.dataset.id){
         case 0:
         app.getInfoOrder('fruit-board','time','desc',e=>{
           this.setData({
             fruitInfo:e.data 
           })
           wx.hideLoading();
         ...
         case 1:
          ...
          break;  
           ...
           case 2:
            app
              .getInfoOrder('fruit-board', 'time', 'desc', e => {
                this.setData({
                  fruitInfo: e.data
                })
  ...
  onShow: function () {
    console.log('show');
    //数据请求  云端请求数据 比较早，放到onReady 晚了 ，请求要花时间，尽量提前，onLoad 正在加载 加载且渲染页面，线程
    // 如果又在启动一个下载的进程的话  可能就会卡到
    //fruit-board
    //封装所有的请求为一个方法
    //app 当前应用
    app.getInfoOrder('fruit-board','time','desc',e=>{
